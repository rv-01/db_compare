...
class OracleTableComparator:
    """Main class for Oracle table comparison"""
    ...
    def _get_table_hash_chunks(self, db_config: DatabaseConfig, table_config: TableConfig) -> List[Dict]:
        """Get table data in chunks with hash values"""
    ...
    def compare_table(self, table_config: TableConfig) -> ComparisonResult:
        ...
        self.audit_logger.log_comparison_start(run_id, table_config.table_name)
        
        try:
            # Get all chunks for both databases
            source_chunks = self._get_table_hash_chunks(self.source_db, table_config)
            target_chunks = self._get_table_hash_chunks(self.target_db, table_config)
            ...
            for i in range(max_chunks):
                ...
                # Collect data for SQL generation
                differences_for_sql.append({
                    'missing_in_target': comparison['missing_in_target'],
                    'missing_in_source': comparison['missing_in_source'],
                    'different_rows': comparison['different_rows'],
                    'source_data': comparison['source_data'],
            ...
            self.audit_logger.log_comparison_end(run_id, table_config.table_name, result)
            return result
            
        except Exception as e:
            self.audit_logger.log_error(run_id, table_config.table_name, str(e))
            raise
