...
class SQLiteDatabase:
    """Handle SQLite database connections and queries."""
    ...
    def get_table_metadata(self, table_name: str) -> Tuple[str, List[str]]:
        ...
        with self.connect() as cursor:
            ...
            for col in columns_info:
                if col[5] == 1:  # SQLite table_info: column 5 is pk flag
...
def compare_tables(config: DatabaseConfig):
    ...
    source_db = SQLiteDatabase(config.config['source_db'])
    target_db = SQLiteDatabase(config.config['target_db'])
...
def main(config_path: str = 'slite_config.json'):
