def main():
    ...
    try:
        ...
        if args.json_output:
            ...
            print(f"   ✅ Tables in sync: {synced_count}")
            print(f"   ⚠️ Tables with differences: {len(results) - synced_count}")
            ...
            if results:
                ...
                for result in results:
                    ...
                    print(f"   {status_icon} {result.get_full_table_name()}: "
                          f"{result.matching_rows:,} matching, "
                          f"{result.different_rows:,} different, "
                          f"{result.missing_in_target:,} missing in target")
        
        print(f"\n📄 Check audit logs: {comparator.audit_logger.audit_file}")
        print(f"📄 Check generated SQL files for synchronization scripts")
